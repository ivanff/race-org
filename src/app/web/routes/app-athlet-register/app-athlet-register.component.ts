...
import {catchError, debounceTime, first, map, skipWhile, take, takeUntil, tap} from "rxjs/operators"
    ...
    onSendSms(): void {
        if (this.getAthletForm.controls['phone'].valid) {
            this.http.post(environment.backend_gateway + '/sms',
                JSON.stringify({phone: this.getAthletForm.controls['phone'].value, competition_id: this.competition.id}),
                {headers: new HttpHeaders({'Content-Type': 'application/json'})}).pipe(
