...
filename="race_org.0.1.${time}.aab"
...
tns build android --release --env.snapshot --env.uglify --env.aot --key-store-path ./key_store/upload_keystore.jks --key-store-password siKiyas9 --key-store-alias upload --key-store-alias-password siKiyas9 --aab --copy-to $filename
cp $filename race_org.latest.aab
